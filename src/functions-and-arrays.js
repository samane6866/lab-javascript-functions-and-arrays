...
function sum(mixedArr) {
  ...
  for (let i = 0; i < mixedArr.length; i++) {
    if (typeof mixedArr[i] === "string") {
      let str = mixedArr[i].split("");
      //for (let i = 0; i < str[i].length; i++){
      totalSumMix += str[i].length;
    } else if (mixedArr[i] === true) {
      totalSumMix += 1;
    } else {
      totalSumMix += mixedArr[i];
    }
  }
  return totalSumMix;
}
...
const anArray = [6, 12, 1, 18, 13, 16, 2, 1, 8, 10];
function averageNumbers(anArray) {
  return sumNumbers(anArray) / anArray.length;
}
console.log(averageNumbers(anArray));
...
function howManyTimes(firstWord, secondWord) {
  let repetitionWord = 0;

  for (let i = 0; i < firstWord.length; i++) {
    if (firstWord[i] === secondWord) {
      repetitionWord++;
    }
  }
